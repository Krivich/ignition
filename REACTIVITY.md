...
The server can inject data into the page in two ways. Choose based on what the page does:

| Variable | Use for | Payload |
|---|---|---|
| `__IGNITION_MANIFEST__` | Read-only content pages: catalogs, articles, landing blocks | Only the slices used by blocks (compact, SEO-friendly) |
| `__IGNITION_INITIAL_DATA__` | Interactive pages: forms, filters, dashboards | Full dataset — everything bindings, actions, and computed values need |

Use `__IGNITION_INITIAL_DATA__` whenever the page has `data-ignition-binding`, reads state paths outside block slices, or uses actions that touch paths not present in the manifest. Use `__IGNITION_MANIFEST__` only when the page is pure blocks and every path the client needs is declared in `data-ignition-data`.

The runtime boot sequence accepts either variable; `__IGNITION_MANIFEST__` is checked first, then `__IGNITION_INITIAL_DATA__`.
...
3. **Create state** — wraps `window.__IGNITION_MANIFEST__` or `window.__IGNITION_INITIAL_DATA__` in a reactive proxy
...
State is created automatically from `__IGNITION_MANIFEST__` or `__IGNITION_INITIAL_DATA__`. You do not call `createReactiveState()` directly — it is done by the boot sequence. But the API is:
