...
Ignition uses **hybrid SSR/CSR**: pages are pre-rendered server-side, then the client runtime "attaches" to existing DOM and makes it interactive. There is no virtual DOM — reactivity works by re-rendering Handlebars templates and replacing block content.

```
Server:  JSON + Template → HTML (sent to browser)
...
Then a page config function:

```html
<script>
window.__IGNITION_PAGE_CONFIG__ = function(state, api) {
    // Register actions, renderers, subscriptions here
};
</script>
```

And reactive blocks in the `<body>`:

```html
<div data-ignition-block="my/template"
     data-ignition-depends="path1, path2">
</div>
```
...
1. **Compile templates** — reads `window.__IGNITION_TEMPLATES__` (generated by the build), lazily compiles each with `Handlebars.compile()`
2. **Create state** — wraps `window.__IGNITION_INITIAL_DATA__` in a reactive proxy
3. **Run page config** — calls `window.__IGNITION_PAGE_CONFIG__(state, api)` — this is where you register everything
4. **Initialize blocks** — finds all `[data-ignition-block]` elements, renders them, and subscribes to their dependencies
5. **Initialize bindings** — finds all `[data-ignition-binding]` elements, sets up two-way binding
6. **Initialize event handlers** — finds all `[data-ignition-on]` elements, attaches click/submit/input listeners
...
State is created automatically from `__IGNITION_INITIAL_DATA__`. You don't call `createReactiveState()` directly — it's done by the boot sequence. But the API is:
...
|-----------|----------|-------------|
| `data-ignition-block="template/name"` | Yes | Which template to render inside this element |
| `data-ignition-depends="path1, path2"` | No | Comma-separated state paths that trigger re-render |

### How It Works

1. On boot, `initBlocks()` finds all `[data-ignition-block]` elements
2. For each block, it calls `renderTemplate(templateName, data)` to get HTML
3. It replaces the block's innerHTML with the rendered HTML via `hydrate()`
4. It subscribes to each dependency path — when any changes, the block re-renders

```html
<!-- This block re-renders when 'cart' changes -->
...
By default, blocks receive the entire state as template data. Usually you want to pass a subset. Use **custom renderers** to control what data reaches each template:

```js
window.__IGNITION_PAGE_CONFIG__ = function(state, api) {
    // This block's template receives { items: [...] }
    api.blockOptions.renderers['catalog/product-list'] = function(s) {
        return s.filteredProducts;
    };

    // This block receives the entire state
    api.blockOptions.renderers['form/contact'] = function(s) {
        return s;
    };
};
```
...
Sometimes a block's data depends on state paths not mentioned in `data-ignition-depends`. Use `sourceDeps` to add extra subscriptions:

```js
// The product-list block depends on 'filteredProducts' (declared in HTML)
// but filteredProducts is computed from 'products' and 'ui'.
// Without sourceDeps, changes to 'products' wouldn't trigger re-render.
...
2. Custom renderer (if any) extracts data from state
...
This means **every re-render creates fresh DOM**. Elements inside a block don't persist across renders. Use `afterHydrate` to restore transient state after each render.
...
For simple derived values from `data-ignition-depends`. For complex computed data, prefer custom renderers — they're simpler and don't require the `*` subscriber overhead.

## Full Example: Live Catalog

This example shows binding, actions, computed data, and multiple blocks working together.
...
### Template (`input/templates/catalog.hbs`)
...
<head>
    ...
    window.__IGNITION_PAGE_CONFIG__ = function(state, api) {
        ...
        // 2. Tell blocks what data to use
        api.blockOptions.renderers['catalog/product-list'] = function(s) {
            return s.filteredProducts;
        };
        api.blockOptions.renderers['catalog/cart-header'] = function(s) {
            return s.cart;
        };
        api.blockOptions.sourceDeps['catalog/product-list'] = ['products', 'ui'];
...
<body>
    <!-- Two-way binding: typing updates state.ui.searchQuery -->
    <input type="search" data-ignition-binding="ui.searchQuery">

    <!-- Two-way binding: select updates state.ui.activeCategory -->
    <select data-ignition-binding="ui.activeCategory">
        <option value="all">All</option>
        <option value="books">Books</option>
    </select>

    <!-- Block: re-renders when filteredProducts changes -->
    <div data-ignition-block="catalog/product-list"
         data-ignition-depends="filteredProducts">
    </div>

    <!-- Block: re-renders when cart changes -->
    <div data-ignition-block="catalog/cart-header"
         data-ignition-depends="cart">
    </div>
</body>
...
notify('ui.searchQuery', '', 'jav')
    ↓
'*' subscriber: state.filteredProducts = filterProducts(state)
    ↓
notify('filteredProducts', ...)
    ↓
Block re-renders: product-list template with { items: [JS Guide] }
    ↓
...
```js
// Works: the system deduplicates 'filteredProducts' notifications
state.subscribe('*', function() {
    state.filteredProducts = recompute(state);
});
```

### 2. Block templates must match the renderer's data shape

If your renderer returns `s.cart` (which is `{ items: [...] }`), the template should reference `items`, not `cart.items`:

```js
// Renderer returns { items: [...] }
api.blockOptions.renderers['my/block'] = function(s) { return s.cart; };
```
...
If you need the template to reference `form.errors.name`, the renderer should return the full state (`return s`), not just the form object.
...
If a block renders `filteredProducts` but `filteredProducts` is computed from `products` + `ui`, you need:
...
Without `sourceDeps`, changing `ui.searchQuery` wouldn't re-render this block because the block only subscribes to `filteredProducts`, and the `*` subscriber handles the `ui` → `filteredProducts` link. But `sourceDeps` ensures the block also re-renders directly when `products` or `ui` change.
...
Register a custom renderer for a block. The returned object is passed as the template data context.
